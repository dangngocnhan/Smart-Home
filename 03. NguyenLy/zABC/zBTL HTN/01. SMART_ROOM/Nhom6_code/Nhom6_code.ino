...
//điều khiển nhiệt độ cho phép bật quạt
const int Start_Stop = 10; 
bool status_Start = 1;  // trạng thái cho phép bật quạt
float NhietDo_DK = 27;  // mặc định > 27 độ C thì cho phép bật
const int tang_T = 9;   // tăng giá trị nhiệt độ mặc định
const int giam_T = 8;   // giảm giá trị mặc định
...
void HC05(){  // thực hiện sau ngắt timer1 
  while(Serial.available()){  //kiểm tra có dữ liệu được gửi về từ HC05
    ...
    if(dataHC05 == '1'){   //data = 1 --> cho phép bật đèn
      checkLed = 1;
...
//      Serial.println("Quat tat");
    ...
    //điều khiển nhiệt độ cho phép bật quạt bằng bluetooth
    if(dataHC05 == '4'){
//      lcd.clear();
...
void setup() {
  ...
  Timer1.attachInterrupt(HC05);       // hàm được gọi khi ngắt timer1 xảy ra
    ...
    lcd.setCursor(1, 1);  lcd.print("  VU DUC HAI");
